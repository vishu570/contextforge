...
async function auditDatabase() {
  ...
  try {
    ...
    const modelCounts = await Promise.all([
      ...
      prisma.itemCategory
        .count()
        .then((count) => ({ model: "ItemCategory", count })),
      prisma.collection
        .count()
        .then((count) => ({ model: "Collection", count })),
      prisma.itemCollection
        .count()
        .then((count) => ({ model: "ItemCollection", count })),
      prisma.source.count().then((count) => ({ model: "Source", count })),
      prisma.import.count().then((count) => ({ model: "Import", count })),
      prisma.stagedItem
        .count()
        .then((count) => ({ model: "StagedItem", count })),
      prisma.version.count().then((count) => ({ model: "Version", count })),
      prisma.optimization
        .count()
        .then((count) => ({ model: "Optimization", count })),
      prisma.conversion
        .count()
        .then((count) => ({ model: "Conversion", count })),
      prisma.auditLog.count().then((count) => ({ model: "AuditLog", count })),
      prisma.workflowQueue
        .count()
        .then((count) => ({ model: "WorkflowQueue", count })),
      console
        .log("=".repeat(50))
        .log("=".repeat(50))
        .log("=".repeat(50))
        .count()
        .then((count) => ({ model: "FolderSuggestion", count })),
      prisma.itemEmbedding
        .count()
        .then((count) => ({ model: "ItemEmbedding", count })),
      prisma.semanticCluster
        .count()
        .then((count) => ({ model: "SemanticCluster", count })),
      prisma.contextTemplate
        .count()
        .then((count) => ({ model: "ContextTemplate", count })),
      prisma.contextGeneration
        .count()
        .then((count) => ({ model: "ContextGeneration", count })),
      prisma.modelOptimization
        .count()
        .then((count) => ({ model: "ModelOptimization", count })),
      prisma.contentSummary
        .count()
        .then((count) => ({ model: "ContentSummary", count })),
      prisma.semanticSearch
        .count()
        .then((count) => ({ model: "SemanticSearch", count })),
      prisma.scheduledExport
        .count()
        .then((count) => ({ model: "ScheduledExport", count })),
      prisma.optimizationResult
        .count()
        .then((count) => ({ model: "OptimizationResult", count })),
      prisma.optimizationQueue
        .count()
        .then((count) => ({ model: "OptimizationQueue", count })),
    ])
    ...
    const nullCounts = {
      ...
      sourceMetadata: items.filter(
        (i) => !i.sourceMetadata || i.sourceMetadata === "{}"
      ).length,
    ...
    Object.entries(nullCounts).forEach(([field, nullCount]) => {
      ...
      const status =
        nullCount === 0 ? "✅" : nullCount === items.length ? "❌" : "⚠️"
      console.log(
        `   ${status} ${field.padEnd(18)} ${nullCount.toString().padStart(3)}/${
          items.length
        } (${percentage}%) null`
      )
    })
    ...
    if (tagCount > 0 && categoryCount > 0) {
      ...
      if (overlap.length > 0) {
        ...
        if (overlap.length > 5)
          console.log(`      ... and ${overlap.length - 5} more`)
      }
    }
    ...
    imports.forEach((imp, i) => {
      console.log(
        `   Import ${i + 1}: ${imp.status} (${imp.sourceType}) - ${
          imp.processedFiles
        }/${imp.totalFiles} files`
      )
    })
    ...
    sources.forEach((source, i) => {
      console.log(
        `   Source ${i + 1}: ${source.type} (${source._count.items} items) - ${
          source.url || "No URL"
        }`
      )
    })
    ...
    if (tagCount > 0 && categoryCount > 0) {
      console.log(
        "   2. Consolidate tag systems - choose Tag+ItemTag OR Category+ItemCategory"
      )
    }
