...
async function startContextForgeSwarm() {
  ...
  try {
    ...
    console.log(`🏥 Health Status: ${result.healthStatus?.overall || 'Unknown'}`);
    console.log(`🧩 Components Initialized: ${result.componentsInitialized.length}`);
    ...
    console.log('   🔧 Backend Team: Real-time processing & system architecture');
    ...
    Object.entries(metrics.teamUtilization).forEach(([team, utilization]) => {
      ...
      const bar = '█'.repeat(Math.floor(utilization * 20)) + '░'.repeat(20 - Math.floor(utilization * 20));
    ...
    activeTasks.slice(0, 5).forEach((task, index) => {
      ...
      console.log(`      Priority: ${task.priority} | Team: ${task.assignedTeam || 'Unassigned'}`);
    });
    ...
    if (result.healthStatus) {
      console.log('\n🏥 SYSTEM HEALTH:');
      console.log(`   Overall: ${getHealthIcon(result.healthStatus.overall)} ${result.healthStatus.overall}`);
      console.log(`   Communication: ${getHealthIcon(result.healthStatus.components.communication)} ${result.healthStatus.components.communication}`);
      console.log(`   Orchestration: ${getHealthIcon(result.healthStatus.components.orchestration)} ${result.healthStatus.components.orchestration}`);
      console.log(`   Resources: ${getHealthIcon(result.healthStatus.components.resources)} ${result.healthStatus.components.resources}`);
    ...
    console.log(`   Status: ${getHealthIcon(commHealth.status)} ${commHealth.status}`);
    ...
    console.log(`   Feedback Loops Active: ${monitoringStats.feedbackLoopsActive}`);
    ...
    console.log('\n💡 USEFUL COMMANDS:');
    console.log('   - View swarm status: npm run swarm:status');
    console.log('   - Monitor metrics: npm run swarm:metrics');
    console.log('   - Health check: npm run swarm:health');
    console.log('   - Stop swarm: npm run swarm:stop');
...
async function displayPeriodicUpdate() {
  ...
  console.log(`   Tasks: ${metrics.totalTasks} total, ${metrics.completedTasks} completed, ${metrics.blockedTasks} blocked`);
  console.log(`   Health: ${getHealthIcon(health.overall)} ${health.overall}`);
  console.log(`   Communication: ${commHealth.metrics.queueSize} messages in queue`);
...
export { startContextForgeSwarm, gracefulShutdown };