...
export async function parseContent(
  ...
  const items = await parseFile(filepath, content)
  return (
    items[0] || {
      name: filepath.split("/").pop() || "untitled",
      content,
      type: "other",
      format: "." + (filepath.split(".").pop() || "txt"),
      metadata: {},
    }
  )
}
...
async function parseMarkdown(
  ...
  if (Object.keys(parsed.data).length > 0) {
    const type =
      detectTypeFromMetadata(parsed.data) ||
      detectTypeFromContent(parsed.content)
...
async function parseXml(
  ...
  const extractItems = (obj: any, prefix = ""): void => {
    if (obj.prompts?.prompt) {
      const prompts = Array.isArray(obj.prompts.prompt)
        ? obj.prompts.prompt
        : [obj.prompts.prompt]
    ...
    if (obj.agents?.agent) {
      const agents = Array.isArray(obj.agents.agent)
        ? obj.agents.agent
        : [obj.agents.agent]
      agents.forEach((agent: any, index: number) => {
        items.push({
          name:
            agent.$.name || agent.$.title || `${filename}_agent_${index + 1}`,
...
function detectTypeFromContent(content: string): ParsedItem["type"] {
  ...
  if (
    ...
    lowerContent.includes("boilerplate")
  ) {
    return "template"
  }
  if (
    lowerContent.includes("snippet") ||
    ...
    parsed.data.forEach((row: any, index: number) => {
      try {
        ...
        const item: ParsedItem = {
          ...
          type:
            mappedData.type || detectTypeFromContent(mappedData.content || ""),
          format: "csv",
          metadata: {
            ...mappedData.metadata,
            csvRow: index + 1,
...
export function detectTypeFromContentAdvanced(
  content: string,
  metadata?: Record<string, any>
): {
  type: ParsedItem["type"]
  ...
  for (const pattern of patterns) {
    ...
    if (score > maxScore) {
      ...
      if (matchedPatterns.length > 0) {
        reasons.push(
          `Matched ${pattern.type} patterns: ${matchedPatterns.length}`
        )
      }
    }
  }
