...
Respond in JSON format with fields: type, confidence, reasoning`
  ...
  async optimizeForModel(
    ...
    const modelSpecificInstructions = {
      openai:
        "Optimize for GPT-4/ChatGPT: focus on clear instructions, use system/user/assistant roles effectively",
      anthropic:
        "Optimize for Claude: emphasize clarity, use XML tags for structure, be explicit about requirements",
      gemini:
        "Optimize for Gemini: use clear formatting, provide examples, structure with markdown",
    }
  ...
  private heuristicClassification(content: string): ClassificationResult {
    ...
    if (
      ...
      return {
        ...
        confidence: 0.5,
        reasoning:
          "Contains function, const, or class keywords indicating code",
        metadata: { heuristic: true },
      }
    }
