...
const FOLDER_TEMPLATES = {
  prompts: {
    "/prompts/development/backend": [
      "API development",
      "Database queries",
      "Server-side logic",
    ],
    "/prompts/development/frontend": [
      "UI components",
      "User interactions",
      "Client-side logic",
    ],
    "/prompts/development/devops": ["Deployment", "Infrastructure", "CI/CD"],
    "/prompts/content/writing": [
      "Blog posts",
      "Documentation",
      "Creative writing",
    ],
    "/prompts/content/marketing": [
      "Social media",
      "Email campaigns",
      "Copy writing",
    ],
  ...
  rules: {
    "/rules/coding-standards/javascript": [
      "ES6+ standards",
      "React patterns",
      "Node.js best practices",
    ],
    "/rules/coding-standards/python": [
      "PEP 8",
      "Type hints",
      "Testing patterns",
    ],
    "/rules/coding-standards/general": [
      "Code review",
      "Git workflow",
      "Documentation",
    ],
  ...
  templates: {
    ...
    "/templates/documentation/user": [
      "User guides",
      "Tutorials",
      "FAQ templates",
    ],
    "/templates/communication/email": [
      "Welcome emails",
      "Newsletters",
      "Notifications",
    ],
    "/templates/communication/social": [
      "Post templates",
      "Campaign formats",
      "Response templates",
    ],
...
export async function generateFolderSuggestions(
  ...
  if (analysisType === "semantic" || analysisType === "auto") {
    ...
    const semanticSuggestions = await generateSemanticSuggestions(
      items,
      existingFolders
    )
    ...
    Object.entries(templates).forEach(([folderPath, keywords]) => {
      const matchingItems = typeItems.filter((item) =>
        keywords.some(
          (keyword) =>
            ...
            (item.subType &&
              item.subType.toLowerCase().includes(keyword.toLowerCase()))
        )
      )
      ...
      if (matchingItems.length > 0) {
        ...
        suggestions.push({
          ...
          confidence: Math.min(
            0.9,
            0.6 + (matchingItems.length / typeItems.length) * 0.3
          ),
          itemIds: matchingItems.map((item) => item.id),
          suggestedName: folderPath.split("/").pop(),
          description: `Auto-suggested folder for ${keywords.join(
            ", "
          )} related items`,
        })
      }
  ...
  try {
    ...
    for (const [type, typeItems] of Object.entries(itemsByType)) {
      ...
      semanticGroups.forEach((group: any) => {
        ...
        const suggestedPath = generatePathFromGroup(
          group,
          basePath,
          existingFolders
        )
...
function generatePathFromGroup(
  ...
  if (existingFolders.some((f) => f.path === suggestedPath)) {
    ...
    let counter = 1
    while (
      existingFolders.some((f) => f.path === `${suggestedPath}-${counter}`)
    ) {
      counter++
    }
