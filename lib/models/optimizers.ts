...
import { LLMService, LLMProvider } from '@/lib/llm';
...
export class ModelOptimizer {
  ...
  async optimizeForModel(
    ...
    const maxBudget = options.maxTokenBudget || Math.floor(modelConfig.contextWindow * 0.8);
    ...
    if (originalTokens.total > maxBudget || options.aggressiveOptimization) {
      ...
      if (!options.preserveFormatting) {
        const whitespaceResult = this.removeRedundantWhitespace(optimizedContent);
      ...
      const compressionResult = this.compressRepetitivePatterns(optimizedContent);
      ...
      const currentTokens = await this.countTokens(optimizedContent, modelConfig);
    ...
    const optimizedTokens = await this.countTokens(optimizedContent, modelConfig);
    const tokenSavings = originalTokens.total - optimizedTokens.total;
    const costSavings = this.calculateCostSavings(originalTokens, optimizedTokens, modelConfig);
   ...
   */
  async countTokens(content: string, modelConfig: ModelConfig): Promise<TokenCount> {
   ...
   */
  private removeRedundantWhitespace(content: string): { content: string; tokenSavings: number } {
   ...
   */
  private compressRepetitivePatterns(content: string): { content: string; tokenSavings: number } {
    const original = content;
    ...
    for (const [phrase, count] of phraseMap.entries()) {
      if (count >= 3 && phrase.length > 20) {
        ...
        const abbrev = phrase.split(' ').map(w => w[0]).join('');
        optimized = optimized.replace(
        ...
        optimized = optimized.replace(
          new RegExp(`\\b${phrase.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')} \\(${abbrev}\\)`, 'g'),
          phrase,
  ...
  private async applyModelSpecificFormatting(
    ...
    if (modelConfig.formatPreferences) {
      ...
      if (modelConfig.formatPreferences.useXmlTags && modelConfig.provider === 'anthropic') {
        // Convert markdown headers to XML tags
        optimized = optimized.replace(/^#{1,6}\s+(.+)$/gm, '<section>$1</section>');
      ...
      if (modelConfig.formatPreferences.roleBasedPrompting && modelConfig.provider === 'openai') {
...
Provide the trimmed version that maintains quality while reducing token count:`;
      ...
      const tokenSavings = Math.ceil((content.length - trimmedContent.length) / 4);
  ...
  private calculateCostSavings(
    ...
    const originalCost = (originalTokens.total * modelConfig.inputCostPer1k) / 1000;
    const optimizedCost = (optimizedTokens.total * modelConfig.inputCostPer1k) / 1000;
   ...
   */
  async getOptimization(itemId: string, targetModel: string): Promise<OptimizationResult | null> {
    try {
  ...
  async getCostEstimates(
    content: string,
    models: string[]
  ): Promise<Record<string, { tokens: number; inputCost: number; outputCost: number }>> {
    const estimates: Record<string, { tokens: number; inputCost: number; outputCost: number }> = {};
  ...
  async getModelRecommendations(
    ...
    for (const [modelId, config] of Object.entries(MODEL_CONFIGS)) {
      ...
      if (requirements.prioritizeQuality) {
        ...
        } else if (config.provider === 'openai' && config.model.includes('gpt-4')) {
          score += 0.35;
      ...
      } else if (!requirements.requiresLargeContext && contextUtilization < 0.5) {
        score += 0.1;
