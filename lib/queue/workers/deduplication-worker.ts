...
export class DeduplicationWorker extends BaseWorker<DeduplicationJobData> {
  ...
  async process(
    ...
    try {
      ...
      const structuralSimilarities = await this.findStructuralSimilarities(items);
      ...
      const semanticSimilarities = await this.findSemanticSimilarities(items, validatedData.threshold);
      ...
      const groups = this.groupDuplicates(items, similarities, validatedData.threshold);
      ...
      throw new Error(`Deduplication failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private findExactDuplicates(items: Array<{ id: string; content: string; name: string }>): SimilarityResult[] {
  ...
  private async findStructuralSimilarities(
    ...
    for (let i = 0; i < items.length - 1; i++) {
      for (let j = i + 1; j < items.length; j++) {
        const similarity = this.calculateStructuralSimilarity(items[i].content, items[j].content);
        
        if (similarity > 0.7) { // High structural similarity threshold
          results.push({
            itemId1: items[i].id,
            itemId2: items[j].id,
            similarity,
  ...
  private calculateStructuralSimilarity(content1: string, content2: string): number {
    ...
    let totalElements = Math.max(structure1.elements.length, structure2.elements.length);
    ...
    const lengthSimilarity = 1 - Math.abs(structure1.length - structure2.length) / Math.max(structure1.length, structure2.length);

    // Combined similarity
    return (elementSimilarity * 0.7 + lengthSimilarity * 0.3);
  }

  private extractStructure(content: string): { elements: string[]; length: number } {
  ...
  private async findSemanticSimilarities(
    ...
    for (let i = 0; i < items.length - 1; i++) {
      for (let j = i + 1; j < items.length; j++) {
        try {
          const similarity = await this.calculateSemanticSimilarity(items[i], items[j]);
          ...
          console.warn(`Failed to calculate semantic similarity between ${items[i].id} and ${items[j].id}:`, error);
        }
      }
    }
...
Return only the similarity score as a decimal:`;
  ...
  private groupDuplicates(
    ...
    const groups: Array<{ canonical: string; duplicates: string[]; similarity: number }> = [];
    ...
    sortedSimilarities.forEach(similarity => {
      if (!processed.has(similarity.itemId1) && !processed.has(similarity.itemId2)) {
        // Create new group
        ...
        const duplicate = canonical.id === similarity.itemId1 ? similarity.itemId2 : similarity.itemId1;
      ...
      } else if (processed.has(similarity.itemId1) && !processed.has(similarity.itemId2)) {
        // Add to existing group where itemId1 is already processed
        const existingGroup = groups.find(g => 
          g.canonical === similarity.itemId1 || g.duplicates.includes(similarity.itemId1)
        );
        if (existingGroup && !existingGroup.duplicates.includes(similarity.itemId2)) {
        ...
        const existingGroup = groups.find(g => 
          g.canonical === similarity.itemId2 || g.duplicates.includes(similarity.itemId2)
        );
        if (existingGroup && !existingGroup.duplicates.includes(similarity.itemId1)) {
  ...
  private async updateDuplicateRelationships(
    groups: Array<{ canonical: string; duplicates: string[]; similarity: number }>
  ): Promise<void> {
