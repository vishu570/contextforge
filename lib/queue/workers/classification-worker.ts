...
export class ClassificationWorker extends BaseWorker<ClassificationJobData> {
  ...
  async process(
    ...
    try {
      ...
      const targetModels =
        validatedData.targetModels ||
        (await this.determineTargetModels(classification, characteristics))
      ...
      const enhancedMetadata = await this.generateMetadata(
        validatedData.content,
        classification,
        characteristics
      )
      ...
      if (validatedData.itemId) {
        await this.updateItemClassification(
  ...
  private async extractCharacteristics(
    ...
    const characteristics = {
      ...
      hasInstructions:
        /\b(you are|act as|your task|instructions?|follow|do not|must|should)\b/i.test(
          content
        ),
      hasExamples: /\b(example|for instance|such as|e\.g\.|like)\b/i.test(
        content
      ),
      hasConditionals:
        /\b(if|when|unless|otherwise|depending|based on)\b/i.test(content),
      hasPersonality: /\b(tone|style|personality|character|voice)\b/i.test(
        content
      ),
      hasConstraints:
        /\b(limit|maximum|minimum|no more than|at least|exactly)\b/i.test(
          content
        ),
    }
...
Possible subTypes: system, user, assistant, instruction, example, validation, workflow, api, ui, data`
  ...
  private heuristicClassification(content: string, characteristics: any): any {
    ...
    if (characteristics.hasInstructions && characteristics.hasPersonality) {
      ...
      characteristics.length < 200 &&
      !characteristics.hasInstructions
    ) {
      type = "snippet"
      confidence = 0.6
    }
  ...
  private async determineTargetModels(
    classification: any,
    characteristics: any
  ): Promise<string[]> {
  ...
  private calculateQualityScore(characteristics: any): number {
    ...
    if (characteristics.wordCount > 50 && characteristics.wordCount < 500)
      score += 0.1
