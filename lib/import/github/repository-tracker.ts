...
export class GitHubRepositoryTracker {
    ...
    if (!source) {
      ...
      source = await prisma.source.create({
        data: {
          ...
          branch,
          pathGlob:
            "**/*.md,**/*.json,**/*.yaml,**/*.yml,**/*.xml,**/*.prompt,**/*.agent,**/*.af,**/*.mdc",
          lastImportedAt: null,
    ...
    try {
      ...
      for (const file of comparison.data.files || []) {
        ...
        const change: FileChange = {
          ...
          sha: file.sha || "",
          previousSha: file.status === "modified" ? undefined : undefined, // Previous SHA not available from comparison API
        }
   ...
   */
  async getRepositorySyncStatus(sourceId: string): Promise<{
    isUpToDate: boolean
   ...
   */
  async getTrackedRepositories(userId: string): Promise<
    Array<{
      sourceId: string
      owner: string
      name: string
    ...
    try {
      ...
      for (const source of sources) {
        ...
        try {
          ...
          console.warn(
            `Failed to get sync status for ${source.repoOwner}/${source.repoName}:`,
            error
          )
...
export const createRepositoryTracker = (githubToken: string) =>
  new GitHubRepositoryTracker(githubToken)
