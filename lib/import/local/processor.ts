...
export class LocalImportProcessor {
  ...
  private async scanFiles(
    ...
    for (const filePath of filePaths) {
      await this.scanPath(filePath, basePath, allFiles, {
        fileExtensions,
        excludePaths,
      })
    }
  ...
  private async scanPath(
    ...
    try {
      ...
      // Apply exclude filters - use path segment matching to avoid false positives
      if (
  ...
  private async createCollectionStructure(
    ...
    for (const dir of sortedDirs) {
      autoCategorize: boolean
      const { userId, importId, sourceId, collectionId, autoCategorize } =
        context
      const parentDir =
        dirParts.length > 1 ? dirParts.slice(0, -1).join(path.sep) : null
      const parentId = parentDir
        ? collectionMap.get(parentDir)
        : rootCollectionId
  ...
  private async processFile(
    ...
    const { userId, importId, sourceId, collectionId, autoCategorize } = context
    ...
    if (autoCategorize) {
      await this.autoCategorizeItem(item.id, file.content, userId)
    }
  }

  private async autoCategorizeItem(
    itemId: string,
    content: string,
    userId: string
  ): Promise<void> {
  ...
  private determineItemType(filePath: string, parsedContent: any): string {
    ...
    if (
      ...
      (lowerContent.includes("role") || lowerContent.includes("persona"))
    ) {
      return "agent"
    }
