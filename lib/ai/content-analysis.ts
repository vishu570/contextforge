...
export class ContentAnalysisService {
   ...
   */
  async analyzeContent(itemId: string, content: string): Promise<ContentInsights> {
    ...
    const recommendations = await this.generateRecommendations(summary, quality, tags);
...
Suggest 2-5 broad categories for folder organization.`;
  ...
  private async generateRecommendations(
    ...
    if (quality.overallScore < 0.7) {
      recommendations.push('Consider improving content quality based on the assessment feedback');
    }
    if (quality.clarity < 0.6) {
      recommendations.push('Improve clarity by simplifying language and adding more explanations');
    }
    if (quality.structure < 0.6) {
      recommendations.push('Better organize content with clear headings and logical flow');
    }

    // Complexity-based recommendations
    if (summary.complexity === 'complex' && summary.readabilityScore < 0.5) {
      recommendations.push('Consider breaking down complex content into smaller, digestible sections');
    }

    // Tag-based recommendations
    const hasComplexityTag = tags.tags.some(tag => tag.category === 'complexity');
    if (!hasComplexityTag) {
      recommendations.push('Add complexity level tags to help users find appropriate content');
    }
    ...
    if (summary.wordCount < 50) {
      recommendations.push('Consider expanding the content to provide more value and context');
    }
    if (summary.wordCount > 2000) {
      recommendations.push('Consider breaking long content into multiple focused pieces');
    }
  ...
  private async storeAnalysisResults(
    ...
    try {
      ...
      for (const tag of tags.tags) {
        if (tag.confidence >= 0.7) { // Only create high-confidence tags
   ...
   */
  private async createTagIfNotExists(tagName: string, category: string): Promise<void> {
    try {
  ...
  private generateFallbackSummary(content: string): ContentSummary {
    ...
    return {
      summary: sentences[0]?.substring(0, 200) + '...' || 'Content summary not available',
   ...
   */
  private generateFallbackQualityAssessment(content: string): QualityAssessment {
    const words = content.split(/\s+/);
