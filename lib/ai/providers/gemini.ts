...
export class GeminiProvider implements AIProvider {
  ...
  public readonly models = ['gemini-2.0-flash-exp', 'gemini-1.5-pro', 'gemini-1.5-flash', 'text-embedding-004']
  ...
  async optimize(content: string, type: OptimizationType, options: OptimizationOptions = {}): Promise<OptimizationResult> {
    ...
    try {
      ...
      throw new ProviderError(
        `Gemini optimization failed: ${error.message}`,
        this.name,
        error.code,
        error.status
      )
  ...
  async categorize(content: string, options: CategorizationOptions = {}): Promise<string[]> {
    try {
      ...
      if (!response) {
        throw new ProviderError('No categorization result from Gemini', this.name)
      }
  ...
  async isHealthy(): Promise<boolean> {
    try {
      const genModel = this.client.getGenerativeModel({ model: 'gemini-1.5-flash' })
  ...
  private buildCategorizationPrompt(content: string, options: CategorizationOptions): string {
    let prompt = `Analyze the following content and suggest ${options.maxSuggestions || 5} appropriate categories:\n\n${content}\n\n`
  ...
  private calculateCost(model: string, tokenUsage?: { input: number; output: number }): number {
    ...
    const rates = pricing[model] || pricing['gemini-2.0-flash-exp']
