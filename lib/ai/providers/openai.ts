...
export class OpenAIProvider implements AIProvider {
  ...
  public readonly models = ['gpt-5-2025-08-07', 'gpt-4o', 'gpt-4o-mini', 'text-embedding-3-small', 'text-embedding-3-large']
  ...
  async categorize(content: string, options: CategorizationOptions = {}): Promise<string[]> {
    try {
      ...
      if (!result) {
        throw new ProviderError('No categorization result from OpenAI', this.name)
      }
  ...
  private calculateCost(model: string, tokenUsage?: { input: number; output: number }): number {
    ...
    const pricing = {
      'gpt-5-2025-08-07': { input: 0.10 / 1000, output: 0.30 / 1000 },
