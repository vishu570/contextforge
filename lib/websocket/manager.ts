...
export class WebSocketManager {
   ...
   * Handle authenticated messages
   */
  private async handleAuthenticatedMessage(ws: AuthenticatedWebSocket, message: WebSocketMessage): Promise<void> {
    switch (message.type) {
  ...
  private handleDisconnection(ws: AuthenticatedWebSocket): void {
    ...
    console.log(`WebSocket disconnected for user: ${ws.userId || 'unauthenticated'}`);
  }
   ...
   */
  public notifyJobCreated(jobId: string, jobType: string, userId: string): void {
  ...
  public notifyJobProgress(jobId: string, progress: any): void {
    ...
    redis.setex(`job_progress:${jobId}`, 300, JSON.stringify({
      ...progress,
      timestamp: new Date(),
    }));
  ...
  public notifyJobRetry(jobId: string, error: string, retryCount: number): void {
   ...
   * Send activity feed
   */
  private async sendActivityFeed(ws: AuthenticatedWebSocket, params: any): Promise<void> {
    try {
      ...
      this.sendMessage(ws, {
        ...
        data: { activities: activityFeed, hasMore: activities.length === limit },
        timestamp: new Date(),
      });
    } catch (error) {
   ...
   */
  public sendNotification(userId: string, notification: Omit<NotificationData, 'id' | 'createdAt'>): void {
   ...
   */
  private async handleAnalyticsPing(ws: AuthenticatedWebSocket, data: any): Promise<void> {
   ...
   * Record user activity for analytics
   */
  private async recordUserActivity(userId: string, activityType: string): Promise<void> {
    try {
      const activity = {
        type: activityType,
      ...
      await redis.lpush(`user:${userId}:recent_activity`, JSON.stringify(activity));
      await redis.ltrim(`user:${userId}:recent_activity`, 0, 19);
      await redis.expire(`user:${userId}:recent_activity`, 86400);

      // Update user metrics
      await redis.hset(`user:${userId}:realtime_metrics`, 'last_activity', activity.timestamp);
    } catch (error) {
   ...
   */
  public broadcastAnalyticsEvent(userId: string, eventType: string, data: any): void {
    const userConnections = this.clients.get(userId);
