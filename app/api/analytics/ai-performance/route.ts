...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalRequests = Object.values(modelUsageData).reduce((sum, stats: any) => sum + stats.total, 0);
    const totalCompleted = Object.values(modelUsageData).reduce((sum, stats: any) => sum + stats.completed, 0);
    const overallSuccessRate = totalRequests > 0 ? totalCompleted / totalRequests : 0;
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        totalCost: Object.values(costAnalysis).reduce((sum: number, cost: any) => sum + cost.totalCost, 0)
      }
    });
  } catch (error) {
...
async function getModelUsage(userId: string, startDate: Date) {
  try {
    ...
    optimizationStats.forEach(stat => {
      ...
      if (!modelUsage[model]) {
        modelUsage[model] = { total: 0, completed: 0, failed: 0, pending: 0, processing: 0 };
      }
      
      modelUsage[model][stat.status === 'approved' ? 'completed' : 
                       stat.status === 'rejected' ? 'failed' : 'pending'] += stat._count;
    ...
    queueStats.forEach(stat => {
      ...
      if (!modelUsage[type]) {
        modelUsage[type] = { total: 0, completed: 0, failed: 0, pending: 0, processing: 0 };
      }
      
      modelUsage[type][stat.status] = (modelUsage[type][stat.status] || 0) + stat._count;
...
async function getProcessingMetrics(userId: string, startDate: Date) {
  try {
    ...
    const processingMetrics: Record<string, { totalTime: number; count: number; avgTime: number }> = {};
    ...
    optimizations.forEach(opt => {
      if (opt.reviewedAt) {
        const processingTime = opt.reviewedAt.getTime() - opt.createdAt.getTime();
        
        if (!processingMetrics[opt.targetModel]) {
          processingMetrics[opt.targetModel] = { totalTime: 0, count: 0, avgTime: 0 };
        }
    ...
    Object.keys(processingMetrics).forEach(key => {
      ...
      metrics.avgTime = metrics.count > 0 ? metrics.totalTime / metrics.count : 0;
    });
...
async function getErrorAnalysis(userId: string, startDate: Date) {
  try {
    ...
    rejectedOptimizations.forEach(opt => {
...
async function getCostAnalysis(userId: string, startDate: Date) {
  try {
    ...
    optimizationCosts.forEach(cost => {
    ...
    // Add estimated costs for other operations (mock data for demonstration)
    const estimatedCosts = {
      'semantic_search': { totalCost: 0.05, avgCost: 0.001, requestCount: 50 },
      'content_analysis': { totalCost: 0.12, avgCost: 0.002, requestCount: 60 },
      'classification': { totalCost: 0.08, avgCost: 0.0015, requestCount: 55 }
    };

    Object.entries(estimatedCosts).forEach(([operation, cost]) => {
      if (!costAnalysis[operation]) {
        costAnalysis[operation] = cost;
      }
    });
...
async function getQualityMetrics(userId: string, startDate: Date) {
  try {
    ...
    return {
      ...
      contentQuality: { avgConfidence: 0, avgReadability: 0, avgSentiment: 0, count: 0 }
    };
  }
}
