...
export class EnhancedLLMService {
  ...
  private saveCustomEndpoints() {
    ...
    localStorage.setItem('contextforge-custom-endpoints', JSON.stringify(endpoints));
  }
  ...
  async generateChatCompletion(
    ...
    try {
      ...
      if (config.provider === 'custom' && config.customEndpoint) {
        result = await this.generateWithCustomEndpoint(messages, config, config.customEndpoint);
      } else {
  ...
  private async generateWithOpenAI(
    ...
    return {
      ...
      finishReason: choice.finish_reason === 'stop' ? 'stop' : 
                   ...
                   choice.finish_reason === 'function_call' ? 'function_call' : 'stop',
      usage: {
    ...
    for await (const chunk of stream) {
      ...
      if (chunk.choices[0]?.finish_reason) {
        yield {
          content,
          isComplete: true,
          functionCall: functionCall ? { ...functionCall, isComplete: true } : undefined,
          usage: {
  ...
  async batchGenerate(requests: BatchRequest[]): Promise<BatchResult[]> {
    const results = await Promise.allSettled(
      requests.map(async (request) => {
        try {
          const result = await this.generateChatCompletion(request.messages, request.config);
  ...
  async compareModels(
    ...
    const requests: BatchRequest[] = models.map(modelId => ({
      id: modelId,
      messages,
      config: { ...config, model: modelId, provider: this.getModelProvider(modelId) }
    }));
    ...
    const scoredResults = results.map(result => {
      if (result.error) {
        return { model: result.id, result: {} as GenerationResult, rank: models.length, score: 0 };
      }
    ...
    const avgCost = validResults.reduce((sum, r) => sum + r.result.cost, 0) / validResults.length;
    const avgDuration = validResults.reduce((sum, r) => sum + r.result.duration, 0) / validResults.length;
    const avgTokens = validResults.reduce((sum, r) => sum + r.result.usage.totalTokens, 0) / validResults.length;
  ...
  private calculateCost(usage: { totalTokens: number }, model: ModelConfig): number {
    return (usage.totalTokens / 1000) * model.cost;
  }

  private getModelConfig(modelId: string): ModelConfig | undefined {
    return this.models.find(m => m.id === modelId) || this.models.find(m => m.isDefault);
  }

  private getModelProvider(modelId: string): 'openai' | 'anthropic' | 'google' | 'custom' {
    const model = this.getModelConfig(modelId);
  ...
  async executeFunctionCall(
    ...
    const functionDef = availableFunctions.find(f => f.name === functionCall.name);
    if (!functionDef) {
